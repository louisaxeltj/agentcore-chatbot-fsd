...
def lambda_handler(event, context):
   try:
      ...
      print(f"Requirements (first 50 chars): {requirements_text[:50]}")
      print(f"Flow (first 50 chars): {flow_text[:50]}")
      print(f"Tree (first 50 chars): {tree_text[:50]}")


      flow_comparison_table, input_token, output_token = compare_requirements_flow(bedrock, model_id, requirements_text, flow_text)
      tree_comparison_table, input_token, output_token = compare_requirements_tree(bedrock, model_id, requirements_text, tree_text)
...
def compare_requirements_tree(bedrock, model_id, requirements, tree):
    prompt = f"""
...
def compare_requirements_flow(bedrock, model_id, requirements, flow):
    prompt = f"""
